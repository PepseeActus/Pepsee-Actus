...
$key_a = 'field_5f7585c3af587';
$key_b = 'field_5f750abe629fc';

// Add the filter to the first relationship field
// The key must match $key_a above
add_filter(
    'acf/update_value/key=field_5f7585c3af587',
...
add_filter(
    'acf/update_value/key=field_5f750abe629fc',
    function ($value, $post_id, $field) use ($key_a, $key_b) {
        return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
    },
    10, 5
...
$key_a = 'field_5f7585ecaf588';
$key_b = 'field_5f750b81626ee';

// Add the filter to the first relationship field
// The key must match $key_a above
add_filter(
    'acf/update_value/key=field_5f7585ecaf588',
    function ($value, $post_id, $field) use ($key_a, $key_b) {
        return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
    },
    10, 5
...
// The key must match $key_b above
add_filter(
    'acf/update_value/key=field_5f750b81626ee',
    function ($value, $post_id, $field) use ($key_a, $key_b) {
        return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
    },
    10, 5
...
/**
 * Reciprocal relationship example 3
 * Between two relationship fields which belong to different post types
 */
// Set the field keys for the two relationship fields
// In this example the fileds belong to different post types
$key_a = 'field_5f758639af589';
$key_b = 'field_5f750aec629fd';

// Add the filter to the first relationship field
// The key must match $key_a above
add_filter(
    'acf/update_value/key=field_5f758639af589',
    function ($value, $post_id, $field) use ($key_a, $key_b) {
        return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
    },
    10, 5
...
// The key must match $key_b above
add_filter(
    'acf/update_value/key=field_5f750aec629fd',
    function ($value, $post_id, $field) use ($key_a, $key_b) {
        return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
    },
    10, 5
